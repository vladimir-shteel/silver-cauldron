...
function processDirtyRects(obj_x, obj_y, obj_pos) {
    ...
    for (let i = 0; i < dirtyRects.length; i += BATCH_SIZE) {
        ...
        batch.forEach(rect => {
            ...
            // Process walls - THIS WAS MISSING
...
// Merges overlapping dirty rectangles
function mergeDirtyRects() {
    if (dirtyRects.length <= 1) return;
    
    // Sort rectangles by x coordinate
    dirtyRects.sort((a, b) => a.x - b.x);
    
    const result = [dirtyRects[0]];
...
function processDirtyRectsForNonWallElements(obj_x, obj_y, obj_pos) {
    ...
    for (let i = 0; i < dirtyRects.length; i += BATCH_SIZE) {
        ...
        batch.forEach(rect => {
            // Process shadows for all world objects
            fow_ctx.save();
...
// Init modified to include the initializeFogCoverage procedure
...
function fog_of_war_update(obj) {
    requestAnimationFrame(() => {
        const half_cell = grid_cell_size >> 1;
        const obj_pos = object_position(obj);
        const obj_x = obj_pos.left + half_cell;
        const obj_y = obj_pos.top + half_cell;
        ...
        if (fog_of_war_distance > 0) {
            // Fill with fog pattern first when distance > 0
            fow_ctx.fillStyle = fow_pattern;
            fow_ctx.fillRect(0, 0, fow_canvas.width, fow_canvas.height);

            // Player vision
            addDirtyRect(obj_x, obj_y, fog_of_war_distance);

            // Cache character lights once per frame
            const characterLights = collectCharacterLights(half_cell);
            ...
            cachedElements.lights.forEach(light => {
                if (light.getAttribute('state') === 'on') {
                    ...
                    addDirtyRect(pos.left + half_cell, pos.top + half_cell, radius);
                }
            });

            // Merge overlapping rectangles for better performance
            mergeDirtyRects();
            
            // Process all lights using spatial partitioning approach
            processLightsWithSpatialPartitioning(obj_x, obj_y, characterLights);
            
            // Process non-wall elements using dirty rectangles
            //processDirtyRectsForNonWallElements(obj_x, obj_y, obj_pos);
            processDirtyRects(obj_x, obj_y, obj_pos);
        } else {
            // When distance = 0, add a single dirty rect covering the whole canvas
            addDirtyRect(
                fow_canvas.width / 2, 
                fow_canvas.height / 2, 
                Math.max(fow_canvas.width, fow_canvas.height)
            );
            
            // Make the entire canvas dark
            fow_ctx.fillStyle = fow_pattern;
            fow_ctx.fillRect(0, 0, fow_canvas.width, fow_canvas.height);
        }
    });
...
function collectCharacterLights(half_cell) {
    const characterLights = [];
    $('div.character').each(function() {
        const radius = parseInt($(this).attr('light'));
        if (radius > 0) {
            const pos = object_position($(this));
            characterLights.push({
                x: pos.left + half_cell,
                y: pos.top + half_cell,
                radius: radius * grid_cell_size
            });
            addDirtyRect(pos.left + half_cell, pos.top + half_cell, radius * grid_cell_size);
        }
    });
    return characterLights;
}

// Process zone shadows separately for cleaner code
...
function createCachedLight(radius) {
    ...
    // Create gradient at lower resolution - IMPORTANT: order of color stops matters!
...
function draw_light_sphere(pos_x, pos_y, radius, relevantWalls) {
    ...
    l_ctx.clearRect(0, 0, l_canvas.width, l_canvas.height);  // Fixed typo: _ctx → l_ctx
...
//Uses spatial partitioning to process lights more efficiently
function processLightsWithSpatialPartitioning(playerX, playerY, characterLights) {
    // Only rebuild the wall grid if walls have changed
    if (!wallGridCache) {
        createWallGrid();
    }
    ...
    if (fog_of_war_distance > 0) {
        processLightWithSpatialOptimization(playerX, playerY, fog_of_war_distance);
    }
...
function createWallGrid() {
    console.log("Rebuilding wall grid");
